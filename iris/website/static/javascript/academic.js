...
window.onload = function() {
  ...
  if (!allList) {
    console.error("Missing .all_list element");
    return;
  }

  const studentInput = document.getElementById('studentId');
  if (!studentInput) {
    console.error("Missing #studentId input element");
    return;
  }
  ...
  fetch(`/academic_list?id=${encodeURIComponent(studentId)}`)
    ...
    .then(data => {
      ...
      sortedYears.forEach(year => {
        ...
        grouped[year].sort((a, b) => parseInt(a.term) - parseInt(b.term));

        grouped[year].forEach(term => {
          const termDiv = document.createElement('div');
          termDiv.className = 'list_term';
          termDiv.id = `view_button_${term.id}`;
          ...
          titleDiv.textContent = `Term ${term.term} Junior Progress Report`;
